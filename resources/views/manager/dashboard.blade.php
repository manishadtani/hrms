...
<style>
    ...
    .section-header a:hover { color: #6d28d9; }
    ...
    .team-item:hover { background: #fafafe; margin: 0 -24px; padding-left: 24px; padding-right: 24px; }
    .team-avatar {
        width: 42px;
        height: 42px;
        border-radius: 12px;
    ...
    .team-info h6 { font-size: 0.88rem; font-weight: 600; color: #1e293b; margin: 0 0 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .team-info small { font-size: 0.75rem; color: #94a3b8; }
...
<div class="mgr-dash">
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-lg-3 fade-up">
            <div class="stat-card">
                ...
                <div class="stat-value count-up" data-target="{{ $teamSize }}">0</div>
                <div class="stat-label">Team Size</div>
                <div class="stat-trend" style="background:#eef2ff;color:#6366f1;">
        ...
        <div class="col-6 col-lg-3 fade-up">
            <div class="stat-card">
                ...
                <div class="stat-trend" style="background:#fff1f2;color:#f43f5e;">
    ...
    {{-- Row 2: Attendance Donut + Team Members + Quick Actions --}}
    <div class="row g-3 mb-4">
        {{-- Team Attendance Donut --}}
        <div class="col-lg-4 fade-up">
            <div class="section-card">
                <div class="section-header">
                    <h6><span class="header-icon icon-emerald"><i class="bi bi-pie-chart-fill"></i></span> Team Attendance</h6>
                </div>
        ...
        <div class="col-lg-4 fade-up">
            <div class="section-card">
                <div class="section-header">
                    <h6><span class="header-icon icon-indigo"><i class="bi bi-person-lines-fill"></i></span> Team Members</h6>
                    <a href="{{ route('manager.team.index') }}">View All</a>
                </div>
                <div class="section-body">
                    @php
                        $memberColors = ['#6366f1','#10b981','#f59e0b','#f43f5e','#06b6d4','#8b5cf6','#ec4899','#14b8a6'];
                    @endphp
                    @forelse($teamMembers->take(6) as $member)
                        @php
                            $mInitials = strtoupper(substr($member->user->name ?? 'U', 0, 1) . substr(explode(' ', $member->user->name ?? 'U')[1] ?? '', 0, 1));
                            $mColor = $memberColors[$loop->index % count($memberColors)];
                        @endphp
                        <div class="team-item">
                            <div class="team-avatar" style="background:{{ $mColor }}15;color:{{ $mColor }};">{{ $mInitials }}</div>
                            <div class="team-info">
                                <h6>{{ $member->user->name ?? 'Unknown' }}</h6>
                                <small>{{ $member->department->name ?? '—' }} · {{ $member->designation->name ?? '—' }}</small>
                            </div>
                        </div>
                    @empty
                        <div class="text-center py-4">
                            <i class="bi bi-people text-muted" style="font-size:2rem;"></i>
                            <p class="text-muted mb-0 mt-2" style="font-size:0.85rem;">No team members found</p>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="section-card">
                ...
                <div class="section-body">
                    <div class="d-flex flex-column gap-2">
                        <a href="{{ route('manager.leaves.index') }}" class="quick-action">
                        ...
                        <a href="{{ route('manager.attendance.index') }}" class="quick-action">
                            <span class="qa-icon icon-emerald"><i class="bi bi-clock-fill"></i></span>
                            Team Attendance
                        </a>
                        <a href="{{ route('manager.attendance.my') }}" class="quick-action">
                            <span class="qa-icon icon-indigo"><i class="bi bi-person-check-fill"></i></span>
                            My Attendance
                        </a>
                        <a href="{{ route('manager.leaves.my') }}" class="quick-action">
                            <span class="qa-icon icon-rose"><i class="bi bi-calendar-heart-fill"></i></span>
                            My Leaves
                        </a>
                    </div>
                </div>
        ...
        <div class="col-lg-5 fade-up">
            <div class="section-card">
                <div class="section-header">
                    ...
                    <a href="{{ route('manager.leaves.index') }}">View All</a>
                </div>
                <div class="section-body">
                ...
                <div class="section-body">
                    @forelse($recentAnnouncements as $announcement)
                        <div class="announce-item">
                            <h6>{{ Str::limit($announcement->title, 40) }}</h6>
                            <p>
