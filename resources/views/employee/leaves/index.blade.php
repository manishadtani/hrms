...
<style>
    ...
    .btn-apply{padding:12px 24px;border:none;border-radius:14px;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;font-weight:700;font-family:'Inter',sans-serif;font-size:.9rem;text-decoration:none;display:flex;align-items:center;gap:8px;transition:all .3s}
    .btn-apply:hover{transform:translateY(-2px);box-shadow:0 10px 30px rgba(99,102,241,.35);color:#fff}
    ...
    .btn-cancel{padding:5px 14px;border:1px solid #fecaca;border-radius:10px;background:#fff;color:#ef4444;font-size:.75rem;font-weight:600;cursor:pointer;transition:all .3s;font-family:'Inter',sans-serif}
...
<div class="leaves-page">
    ...
    <div class="page-header fade-up">
        ...
        <a href="{{ route('employee.leaves.create') }}" class="btn-apply"><i class="bi bi-plus-circle-fill"></i> Apply Leave</a>
    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($lr->status === 'pending')
                                    <form method="POST" action="{{ route('employee.leaves.cancel', $lr) }}" style="display:inline" onsubmit="return confirm('Cancel this leave request?')">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" class="btn-cancel"><i class="bi bi-x-lg"></i> Cancel</button>
                                    </form>
                                @else
